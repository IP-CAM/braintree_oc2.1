<?php
class ControllerPaymentBraintreeTlt extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('payment/braintree_tlt');

		$this->document->setTitle($this->language->get('heading_title'));

		$this->load->model('setting/setting');

		if (file_exists(DIR_SYSTEM . '/braintree/lib/Braintree.php')) {
		...
		$data['text_copyright'] = '&copy; 2015, <a href="http://taiwanleaftea.com" target="_blank" class="alert-link" title="Authentic tea from Taiwan">Taiwanleaftea.com</a>';
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('payment/braintree_tlt', 'token=' . $this->session->data['token'], 'SSL')
		);

		$data['action'] = $this->url->link('payment/braintree_tlt', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('payment/braintree_tlt.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'payment/braintree_tlt')) {
